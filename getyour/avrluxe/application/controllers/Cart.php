...
class Cart extends CI_Controller
{
        ...
        */
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['product_type']) && $_POST['product_type'] != 'digital_product') {
                    ...
                    $product_delivarable = check_cart_products_delivarable($_POST['user_id'], $area_id[0]['area_id'], $zipcode);
    ...
    public function get_delivery_charge()
    {
        ...
        $product_availability = check_cart_products_delivarable($this->data['user']->id, $area_id[0]['area_id'], $zipcode);
    ...
    public function check_product_availability()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $address_id = $this->input->post('address_id', true);
            $area_id = fetch_details('addresses', ['id' => $address_id], ['area_id', 'area', 'pincode']);
            // $zipcode_id = (!empty($area_id) && $area_id > 0) ? fetch_details('areas', ['id' => $area_id[0]['area_id']], 'zipcode_id') : 0;
            $zipcode = $area_id[0]['pincode'];

            $product_delivarable = check_cart_products_delivarable($this->data['user']->id, $area_id[0]['area_id'], $zipcode);
