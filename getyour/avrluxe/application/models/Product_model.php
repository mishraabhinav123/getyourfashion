    ...
    public function get_product_details($flag = NULL)
    {
        ...
        foreach ($pro_search_res as $row) {
            ...
            if ($row['status'] == '1') {
                ...
                $operate .= '<a class="btn action-btn btn-warning btn-xs update_active_status ml-1 mb-1" data-table="products" title="Deactivate" href="javascript:void(0)" data-id="' . $row['pid'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-on"></i></a></div>';
            } else {
                $tempRow['status'] = '<a class="badge badge-danger text-white" >Inactive</a>';
                $operate .= '<a class="btn action-btn btn-secondary mr-1 mb-1 btn-xs update_active_status" data-table="products" href="javascript:void(0)" title="Active" data-id="' . $row['pid'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-toggle-off"></i></a>';
            }
            ...
            $operate .= " <a href='javascript:void(0)' data-id=" . $row['pid'] . " data-toggle='modal' data-target='#product-faqs-modal' class='btn action-btn btn-info btn-xs mr-1 mb-1' title='View FAQs' ><i class='fas fa-question-circle'></i></a></div>";
    ...
    public function get_stock_details()
    {
        ...
        foreach ($products['product'] as $product) {
            ...
            for ($i = 0; $i < count($variants); $i++) {
                $edit = '<a href="javascript:void(0)" class="edit_btn btn btn-success btn-xs mr-1 mb-1" title="Edit" data-id="' . $variants[$i]['id'] . '" data-url="admin/manage_stock/"><i class="fa fa-pen"></i> Edit</a>';
