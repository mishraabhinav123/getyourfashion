...
class Area_model extends CI_Model
{
    ...
    function get_zipcode_list()
    {
        ...
        foreach ($tax_search_res as $row) {
            $row = output_escaping($row);
            $operate = ' <a href="javascript:void(0)" class="edit_btn btn action-btn btn-success btn-xs mr-1 mb-1"  title="Edit" data-id="' . $row['id'] . '" data-url="admin/area/manage_zipcodes"><i class="fa fa-pen"></i></a>';
            $operate .= ' <a  href="javascript:void(0)" class="btn action-btn btn-danger btn-xs mr-1 mb-1"  title="Delete" id="delete-zipcode" data-id="' . $row['id'] . '" ><i class="fa fa-trash"></i></a>';
            $tempRow['id'] = $row['id'];
            $tempRow['zipcode'] = $row['zipcode'];
            $tempRow['operate'] = $operate;
            $rows[] = $tempRow;
    ...
    function get_area_by_city($city_id, $sort = "a.name", $order = "ASC", $search = "", $limit = '', $offset = '')
    {
        ...
        $count_res = $this->db->select(' COUNT(a.id) as `total` ')->where($where)->get('areas a')->result_array();
        ...
        if (!empty($areas)) {
            for ($i = 0; $i < count($areas); $i++) {
                ...
                $zipcode =  fetch_details('zipcodes', '', '', '', '', '', '', 'id', $areas[$i]['zipcode_id']);
                $areas[$i]['pincode'] = $zipcode[0]['zipcode'];
            }
        }
        $bulkData['total'] = $count_res[0]['total'];
