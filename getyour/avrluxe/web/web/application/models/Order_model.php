    ...
    public function get_orders_list(
        ...
        foreach ($user_details as $row) {
            ...
            if (!empty($row['items'])) {
                ...
                $tempRow['local_pickup'] = ($row['is_local_pickup'] == 1) ? '<label class="badge badge-success">Yes</label>' : '<label class="badge badge-danger">NO</label>';
                ...
                if (!$this->ion_auth->is_delivery_boy()) {
                    $operate = '<div><a href=' . base_url('admin/orders/edit_orders') . '?edit_id=' . $row['id'] . ' class="btn action-btn btn-primary btn-xs mr-1 ml-1 mb-1" title="View" ><i class="fa fa-eye"></i></a>';
                    $operate .= '<a href="javascript:void(0)" class="delete-orders btn action-btn btn-danger btn-xs mr-1 ml-1 mb-1" data-id=' . $row['id'] . ' title="Delete" ><i class="fa fa-trash"></i></a></div>';
                    ...
                    if ($row['items'][0]['type'] == 'digital_product' && $row['items'][0]['download_allowed'] == 0) {
                        ...
                        $operate .= ' <a href="javascript:void(0)" class="edit_digital_order_mails btn action-btn btn-warning btn-xs ml-1 mr-1 mb-1" title="Digital Order Mails" data-order_id="' . $row['id'] . '"  data-target="#digital-order-mails" data-toggle="modal"><i class="far fa-envelope-open"></i></a>';
                    }
                } else {
    ...
    public function get_order_tracking_list()
    {
        ...
        foreach ($txn_search_res as $row) {
            ...
            if ($this->ion_auth->is_admin()) {
                $operate = '<a href=' . base_url('admin/orders/edit_orders') . '?edit_id=' . $row['order_id'] . ' class="btn action-btn btn-primary btn-xs mr-1 mb-1" title="View Order" ><i class="fa fa-eye"></i></a>';
            } else {
                $operate = "";
            }
