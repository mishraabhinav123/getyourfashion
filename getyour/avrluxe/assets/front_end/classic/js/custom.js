          ...
          if (!$.isEmptyObject(data.tags)) {
            ...
            $.each(data.tags, function (i, e) {
              tags +=
                ...
                'products/tags/' +
                e +
                '" target="_blank"><span class="badge badge-secondary p-1 mr-1">' +
                e +
                '</span></a>'
            })
...
jQuery(document).ready(function ($) {
  ...
  $('ul.color-style .yellow').click(function () {
    $('#color-switcher').attr(
      'href',
      base_url + 'assets/front_end/classic/css/colors/yellow.css'
    )
    return false
  })
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.error == false) {
          ...
          $.each(result.data, function (i, e) {
            var is_selected =
              e.city_id == city_id && e.id == area_id ? 'selected' : ''
            html +=
              '<option value=' +
              e.id +
              ' ' +
          ...
          $('#edit_area').html(html).trigger('change')
        } else {
          Toast.fire({
            icon: 'error',
            title: result.message
          })
          $('#edit_area').html('<option>No areas available</option>')
        }
      }
    })
  }
...
$('#edit_area').on('change', function (e, area_id) {
  e.preventDefault()
  var area_id1 =
    area_id == '' || area_id == 'undefined' ? $(this).val() : area_id
  $.ajax({
    type: 'POST',
    data: {
      area_id: $(this).val(),
