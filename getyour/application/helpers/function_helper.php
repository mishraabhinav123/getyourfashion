...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL)
{
    ...
    $t->db->select($discount_filter_data . ' (select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type ,
     ...
     p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.description,p.extra_description,p.total_allowed_quantity,p.status,p.deliverable_type,p.deliverable_zipcodes,p.minimum_order_quantity,p.sku,
     ...
     p.video_type, p.video, p.tags, p.warranty_period, p.guarantee_period, p.made_in,p.hsn_code,p.download_allowed,p.download_type,p.download_link,p.pickup_location,p.brand,p.availability,c.name as category_name,tax.percentage as tax_percentage ,tax.id as tax_id ')
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null, $order_type = '', $from_seller = false)
{
    ...
    $search_res = $t->db->select(' o.*, u.username,u.country_code, p.name,p.type,p.download_allowed,a.name as order_recipient_person,pv.special_price,pv.price,oc.delivery_charge as seller_delivery_charge,oc.promo_discount as seller_promo_dicount')
        ->join(' `users` u', 'u.id= o.user_id', 'left')
        ->join(' `order_items` oi', 'o.id= oi.order_id', 'left')
        ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
        ->join('addresses a', 'a.id=o.address_id', 'left')
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        if ((isset($delivery_boy_id) && $delivery_boy_id != null) || (isset($seller_id) && $seller_id != null)) {
            ...
            $order_details[$i]['total_payable'] = strval($item_subtotal +  $order_details[$i]['delivery_charge'] - $order_details[$i]['promo_discount']);
        } else {
...
function check_cart_products_delivarable($user_id, $area_id = 0, $zipcode = "")
{
    $t = &get_instance();
    ...
    if (!empty($cart)) {
        ...
        for ($i = 0; $i < $cart[0]['cart_count']; $i++) {
            ...
            $tmpRow['is_deliverable'] = (!empty($area_id) && $area_id > 0) ?
                is_product_delivarable('area', $area_id, $cart[$i]['product_id'])
                : false;
