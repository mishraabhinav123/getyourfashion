...
class Order_model extends CI_Model
{
    ...
    public function update_order($set, $where, $isjson = false, $table = 'order_items', $fromuser = false)
    {
    ...
    public function place_order($data)
    {
        ...
        //     ->join('categories c', 'p.category_id = c.id', 'left')
        //     ->join('`taxes` tax', 'tax.id = p.tax', 'LEFT')
        //     ->where_in('pv.id', $product_variant_id)->get('product_variants pv')->result_array();
        if (!empty($product_variant)) {
            ...
            // $seller_promocode_discount = $promo_code_discount / $total_parcels;
            $parcel_sub_total = 0.0;
            foreach ($parcels as $seller_id => $parcel) {

                $parcel_sub_total += $parcel['total'];
                // $delivery_charge += intval($parcel['delivery_charge']);
            }
    ...
    {
        $res = $this->db->select('oi.*,ot.courier_agency,ot.tracking_id,ot.url,oi.otp as item_otp,a.name as user_name,oi.id as order_item_id,p.*,v.product_id,o.*,o.email as user_email,o.id as order_id,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.username as uname,oi.status as order_status,p.id as product_id,p.pickup_location as pickup_location,p.slug as product_slug,p.sku as product_sku,v.sku ,p.name as pname,p.type,p.image as product_image,p.is_prices_inclusive_tax,(SELECT username FROM users db where db.id=oi.delivery_boy_id ) as delivery_boy ')
