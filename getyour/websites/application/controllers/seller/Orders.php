                    ...
                    if (isset($user_res[0]) && !empty($user_res[0])) {
                        ...
                        if (!empty($current_delivery_boy[0]) && count($current_delivery_boy) > 1) {
                            for ($i = 0; $i < count($current_delivery_boys); $i++) {
                                $customer_msg = (!empty($custom_notification)) ? 'Hello Dear ' . $user_res[$i]['username'] . ' ' . $custom_notification[0]['message'] .'<br>'. ' ID #' . $order_items[0]['order_id'] .  ' Appliaction Name : ' . $app_name . '' :  'Hello Dear ' . $user_res[$i]['username'] . 'Order status updated to' . $_POST['val'] . ' for order ID #' . $order_items[0]['order_id'] . ' assigned to you please take note of it! Thank you. Regards ' . $app_name . '';
                                $fcmMsg = array(
                            ...
                            if (isset($current_delivery_boys[0]['delivery_boy_id']) && $current_delivery_boys[0]['delivery_boy_id'] == $_POST['deliver_by']) {
                                $customer_msg = (!empty($custom_notification)) ? 'Hello Dear ' . $user_res[0]['username'] . ' ' . $custom_notification[0]['message'] .'<br>'.' ID #' . $order_items[0]['order_id'] . ' Appliaction Name : ' . $app_name . '' :  'Hello Dear ' . $user_res[0]['username'] . 'Order status updated to' . $_POST['val'] . ' for order ID #' . $order_items[0]['order_id'] . ' assigned to you please take note of it! Thank you. Regards ' . $app_name . '';
                                $fcmMsg = array(
                                    'title' => (!empty($custom_notification)) ? $custom_notification[0]['title'] : "Order status updated",
                                    'body' => $customer_msg,
                                    'type' => "order",
