...
class Seller_model extends CI_Model
{
    ...
    function get_sellers_list()
    {
        ...
        foreach ($offer_search_res as $row) {
            ...
            $operate = " <a href='manage-seller?edit_id=" . $row['user_id'] . "' data-id=" . $row['user_id'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
            ...
            $operate .= '<a href="' . base_url('admin/orders?seller_id=' . $row['user_id']) . '" class="btn btn-primary btn-xs mr-1 mb-1" title="View Orders" ><i class="fa fa-eye"></i></a>';
    ...
    function settle_seller_commission($is_date = TRUE)
    {
        ...
        if ($wallet_updated == true) {
            ...
            foreach ($seller_ids as $seller) {
                ...
                $customer_msg = (!empty($custom_notification)) ? 'Hello Dear ' . $user_res[0]['username'] . ' ' . $custom_notification[0]['message'] .'<br>' . ' Appliaction Name : ' . $app_name . '' : 'Hello Dear ' . $user_res[0]['username'] . 'Commission Amount Credited, which orders are delivered. Please take note of it! Regards' . $app_name . '';
